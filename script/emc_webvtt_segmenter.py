...
def lambda_handler(event, context):
    ...
    # Check MediaConvert job completion event for HLS output
    hls_output_not_found = True
    for outputgroup in event['detail']['outputGroupDetails']:
        if outputgroup['type'] == "HLS_GROUP":
            hls_output_not_found = False
            master_manifest_s3_location = outputgroup['playlistFilePaths'][0]
    ...
    for line in vtt_manifest_to_list:
        if "EXTINF" in line:
            duration = int(line.split(",")[0].split(":")[1])
    ...
    for vtt_segment in video_dict:
        ...
        try:
            ...
            s3_put_response = s3.put_object(Body=segmented_vtt_str, Bucket=bucket, Key=new_vtt_key,ContentType='text/vtt',ACL='public-read')
        except Exception as e:
    ...
    try:
        ...
        s3_put_response = s3.put_object(Body=new_vtt_manifest_str, Bucket=bucket, Key=new_vtt_manifest_key,ContentType='application/vnd.apple.mpegurl',ACL='public-read')
    except Exception as e:
    ...
    try:
        ...
        s3_put_response = s3.put_object(Body=new_master_manifest_str, Bucket=bucket, Key=new_master_manifest_key,ContentType='application/vnd.apple.mpegurl',ACL='public-read')
    except Exception as e:
